angular.module("jkuri.datepicker", []).directive("ngDatepicker", ["$document", function(a) {
    ...
    return {
        ...
        link: function(g, e, d, j) {
            ...
            g.checkDates = function(d){
				var selectedDate = new Date(d.year+'-'+d.month+'-'+d.day);
				$today = new Date();
				$yesterday = new Date($today);
				$yesterday.setDate($today.getDate() - 1); 
				if (selectedDate < $yesterday && window.location.hash != '#/report') {
				  return true;
				}            	
				return false;
        	}
        ...
        template: '<div><input type="text" ng-focus="showCalendar()" ng-value="viewValue" class="ng-datepicker-input form-control" placeholder="{{ placeholder }}"></div><div class="ng-datepicker" ng-show="calendarOpened">  <div class="controls">    <div class="left">      <i class="fa fa-backward prev-year-btn" ng-click="prevYear()"></i>      <i class="fa fa-angle-left prev-month-btn" ng-click="prevMonth()"></i>    </div>    <span class="date" ng-bind="dateValue"></span>    <div class="right">      <i class="fa fa-angle-right next-month-btn" ng-click="nextMonth()"></i>      <i class="fa fa-forward next-year-btn" ng-click="nextYear()"></i>    </div>  </div>  <div class="day-names">    <span ng-repeat="dn in dayNames">      <span>{{ dn }}</span>    </span>  </div>  <div class="calendar">    <span ng-repeat="d in days">      <span class="day" ng-click="selectDate($event, d)" ng-class="{disabled: !d.enabled || checkDates(d) }">{{ d.day }}</span>    </span>  </div></div>'
    }
}]);