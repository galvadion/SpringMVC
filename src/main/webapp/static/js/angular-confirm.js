...
(function (root, factory) {
  ...
  angular.module('angular-confirm', ['ui.bootstrap.modal'])
      ...
      .value('$confirmModalDefaults', {
        template: '<div class="modal-header"><h3 class="modal-title custom-font">{{data.title}}</h3></div>' +
        '<div class="modal-body">{{data.text}}</div>' +
        '<div class="modal-footer">' +
        '<button class="btn btn-success btn-ef btn-ef-3 btn-ef-3c" ng-click="ok()"><i class="fa fa-arrow-right"></i>{{data.ok}}</button>' +
        '<button class="btn btn btn-lightred btn-ef btn-ef-4 btn-ef-4c" ng-click="cancel()"><i class="fa fa-arrow-left"></i>{{data.cancel}}</button>' +
        '</div>',
